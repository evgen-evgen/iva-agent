...
| `TELEGRAM_NOTIFICATION_CHAT_ID` | —            | Явный пользовательский адрес для напоминаний, дайджестов, отчётов и предложений обновления. Фолбэка на список входящих пользователей нет. Обычно это ваш личный chat ID. |
| `TELEGRAM_DIAGNOSTIC_CHAT_ID`   | —            | Отдельный админский канал для технических сбоев и эксплуатационных алертов. Напоминания и отчёты пользователя туда не отправляются.                                      |
...
В 10:00 по `ASSISTANT_TIMEZONE` Iva без обращения к модели проверяет Git upstream. Пока новой стабильной версии `MAJOR.MINOR.PATCH` нет, сообщений не будет; каждую новую версию Iva предлагает только один раз. Для доставки всегда используется `TELEGRAM_NOTIFICATION_CHAT_ID`.
