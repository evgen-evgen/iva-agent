...
| `iva notify <text>`                | Save one notification for LibreChat and send the same event to the explicitly configured `TELEGRAM_NOTIFICATION_CHAT_ID` — how cron and `systemd-run` jobs reach both interfaces                                                                                                                         |
