...
`/new` retires only the exact Eve session for the current private chat. That tenant's queued messages are cleared while other tenants keep their histories and queues. `/restart` is owner-only; it performs the same scoped reset first and then restarts `iva.service`. Both commands work while a turn is running. The server-side `iva reset` remains the explicit break-glass command that quarantines the whole workflow store.
